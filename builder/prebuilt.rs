...
//! installation instead of building from source. It handles:
//!
//! - Configuration via environment variables (`PREBUILT_INSTALL_DIR`, etc.)
//! - Validation of installed headers and version compatibility
//! - Library discovery and linking (static/dynamic, with/without prefix)
//! - Binding resolution (pre-generated, internal bindgen, external bindgen-cli)

use crate::{emit_warning, get_aws_lc_include_path, optional_env_crate_target};
use crate::{target_env, target_os, OutputLibType};
...
fn get_bundled_awslc_version() -> String {
...
impl PrebuiltBuilder {
    ...
    pub(crate) fn new(install_dir: PathBuf, prefix: Option<String>) -> Result<Self, String> {
        ...
        // Use CARGO_CFG_TARGET_POINTER_WIDTH (target) rather than cfg!() (host)
        // to correctly handle cross-compilation scenarios.
...
fn find_prebuilt_bindings(config: &Config) -> Option<PathBuf> {
...
fn try_external_bindgen(
    ...
    // non-prebuilt external bindgen behavior in invoke_external_bindgen().
    let options = crate::BindingOptions {
        build_prefix: None,
        include_ssl: cfg!(feature = "ssl"),
        disable_prelude: true,
    ...
    crate::invoke_external_bindgen(manifest_dir, &options, prefix, &bindings_path)
