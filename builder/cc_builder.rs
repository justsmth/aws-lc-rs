...
use crate::{
    ...
    set_env_for_target, should_build_jitter_entropy, target, target_arch, target_env,
    target_is_msvc, target_os, target_vendor, CStdRequested, EnvGuard, OutputLibType,
};
use std::cell::Cell;
...
impl CcBuilder {
    ...
    pub(crate) fn collect_universal_build_options(
        ...
        if target_arch() == "x86" && !is_msvc_target {
    ...
    fn add_defines(&self, build_options: &mut Vec<BuildOption>, is_msvc_target: bool) {
    ...
    ///   3. `CFLAGS`
    fn jitter_entropy_cflags_guards(is_msvc_target: bool) -> Vec<EnvGuard> {
    ...
    fn add_all_files(&self, sources: &[&'static str], cc_build: &mut cc::Build) {
        // dialect -> target ABI; see `target_is_msvc`.
        let is_msvc_target = target_is_msvc();

        let force_include_option = if is_msvc_target { "/FI" } else { "--include=" };
        ...
        if should_build_jitter_entropy() {
            let _je_cflags_guards = Self::jitter_entropy_cflags_guards(is_msvc_target);
