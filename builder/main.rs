...
fn main() {
    ...
    if is_pregenerating_bindings() {
        #[cfg(any(feature = "bindgen", feature = "fips"))]
        {
            ...
            if is_external_bindgen_requested().unwrap_or(false) {
                ...
                invoke_external_bindgen(&manifest_dir, &options, &prefix, &src_bindings_path)
                    .unwrap();
            } else {
    ...
    if !bindings_available {
        if cfg!(feature = "ssl") {
            ...
            let result =
                invoke_external_bindgen(&manifest_dir, &options, &prefix, &gen_bindings_path);
            match result {
                Ok(()) => {
                    emit_rustc_cfg("use_bindgen_pregenerated");
                    bindings_available = true;
...
fn handle_prebuilt_build(manifest_dir: &Path) {
    use prebuilt::PrebuiltBuilder;

    let config =
        prebuilt::get_config().expect("prebuilt::is_enabled() was true but config is None");
    let include_dir = config.install_dir.join("include");

    // 1. Validate installation and detect prefix
    let (_version, prefix) =
        prebuilt::validate_installation(&include_dir, config.skip_version_check)
    ...
    prebuilt::handle_prebuilt_bindings(
        config,
        builder.include_dir(),
        manifest_dir,
        &out_dir(),
    ...
    println!(
        ...
        prebuilt::env_var_crate_target("PREBUILT_INSTALL_DIR")
    );
    println!(
        "cargo:rerun-if-env-changed={}",
        prebuilt::env_var_crate_target("PREBUILT_BINDINGS")
    );
    println!(
        "cargo:rerun-if-env-changed={}",
        prebuilt::env_var_crate_target("PREBUILT_SKIP_VERSION_CHECK")
    );
}
