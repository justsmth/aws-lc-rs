...
mod mem {
    ...
    fn get_layout_offset(size: usize) -> (Layout, usize) {
        let required_layout = Layout::from_size_align(size, MEM_ALIGNMENT)
            .expect(format!("Unable to obtain layout: {size}").as_str());
    ...
    fn write_alloc_data(alloc_ptr: *mut u8, layout: Layout, offset: usize) -> *mut c_void {
        ...
        unsafe {
            let result_ptr = alloc_ptr.add(offset) as *mut c_void;

            let data_ptr = result_ptr.sub(size_of::<AllocData>()) as *mut AllocData;

            data_ptr.write_unaligned(AllocData(alloc_ptr as *mut c_void, layout));
    ...
    fn read_alloc_data(ptr: *mut c_void) -> AllocData {
        unsafe {
            let data_ptr = ptr.sub(size_of::<AllocData>()) as *mut AllocData;
    ...
    fn realloc(ptr: *mut c_void, new_size: usize) -> *mut c_void {
        ...
        unsafe {
            ...
            let realloc_ptr =
                rust_realloc(alloc_data.0 as *mut u8, alloc_data.1, new_layout.size());
    ...
    fn free(ptr: *mut c_void) {
        ...
        unsafe {
            ...
            rust_dealloc(alloc_data.0 as *mut u8, alloc_data.1);
        }
    }
}
